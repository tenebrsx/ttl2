...
const AllProperties = () => {
  ...
  useEffect(() => {
    ...
    setPropertiesData(localProperties);
    setLoading(false);
  }, []);

  const toggleSoldStatus = async (id: string, currentStatus: boolean) => {
    alert("Database functionality disabled. Changes won't be saved.");
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
        <div className="flex items-center">
          ...
          <span className="text-sm text-yellow-800">
            Admin functionality is read-only. Database connection has been
            removed.
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-dusty-clay mt-2">
            Gestiona todas las propiedades del sitio
          </p>
        </div>
        <Link
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="relative">
            <Search
              size={20}
              className="absolute left-3 top-3 text-dusty-clay"
            />
            <input
              type="text"
          ...
          <div className="text-sm text-dusty-clay flex items-center">
            Mostrando {filteredProperties.length} de {propertiesData.length}{" "}
            propiedades
          </div>
        </div>
      </div>
            ...
            <table className="min-w-full divide-y divide-dusty-clay/20">
              ...
              <tbody className="bg-white divide-y divide-dusty-clay/20">
                {filteredProperties.map((property) => (
                  <tr
                    key={property.id}
                    className="hover:bg-cream/50 transition-colors duration-300"
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end space-x-2">
                        ...
                        <button
                          onClick={() =>
                            toggleSoldStatus(property.id, property.is_sold)
                          }
