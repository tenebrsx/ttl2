...
const MapController: React.FC<{ selectedProperty: Property | null | undefined }> = ({ selectedProperty }) => {
  const map = useMap();
  
  useEffect(() => {
    if (selectedProperty) {
      map.setView([selectedProperty.coordinates.lat, selectedProperty.coordinates.lng], 14, {
        animate: true,
        duration: 1
      });
    }
          ...
          >
            {filteredProperties.map((property) => (
              <Marker
                ...
                icon={createCustomIcon(property.type, selectedProperty?.id === property.id)}
                eventHandlers={{
