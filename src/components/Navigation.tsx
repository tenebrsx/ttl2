  ...
  return (
    <>
      <nav className={`fixed w-full z-50 transition-all duration-500 ${
        ...
        <div className="container-max section-padding">
          <div className="flex items-center justify-between h-16 sm:h-20">
            <Link to="/" className="flex items-center group">
              <span className={`font-playfair text-2xl sm:text-3xl font-bold transition-all duration-500 tracking-wide ${
        ...
        {/* Mobile Navigation */}
        <div className={`lg:hidden fixed inset-0 top-16 sm:top-20 z-40 transition-all duration-300 ${
          ...
          {/* Menu Panel */}
          <div className={`relative bg-white/95 backdrop-blur-md border-t border-dusty-clay/10 transform transition-transform duration-300 ${
            ...
            <div className="container-max section-padding py-6 max-h-[calc(100vh-4rem)] overflow-y-auto">
              {navItems.map((item) => (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`block py-4 font-medium text-lg transition-all duration-300 hover:text-deep-copper tracking-wide min-h-[44px] flex items-center ${
                    location.pathname === item.path
                      ? 'text-deep-copper'
                      : 'text-soft-charcoal'
                  }`}
